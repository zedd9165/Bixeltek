...
export default function ShowcaseSection() {
    ...
    return (
        <section className="w-full min-h-screen bg-white text-black gap-5  flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-black ">Here’s a look at some of the websites we’ve crafted — designed for growth, trust, and performance.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-7xl">
                {images.map((item, i) => (
                    <div
                        key={i}
                        className="relative overflow-hidden rounded-2xl pt-4 pb-0 px-2 h-[400px] bg-transparent flex flex-col"
                    >
                        ...
                        {/* name below image */}
                        <div className="p-2 text-center font-inter bg-transparent text-black text-lg font-medium">
                            {item.name}
                        </div>
                    </div>
                ))}
